...
CODER_INTERFACE(ICompressCoder2, 0x18) {
  ...
  (ISequentialInStream *const *inStreams, const UInt64 *const *inSizes,
   UInt32 numInStreams, ISequentialOutStream *const *outStreams,
   const UInt64 *const *outSizes, UInt32 numOutStreams,
...
/*
  ...
  returns:
    ...
    another error code : some error. For example, it can be error code received
  from inStream or outStream function.
  ...
  Parameters:
    ...
    if (inSize != NULL)
    {
      ...
      Some decoder in 7-Zip check it, if (full_decoding mode was set via
  ...
  {
    1) try to set full_decoding mode with
  ICompressSetFinishMode::SetFinishMode(1); 2) call the Code() function with
  specified (inSize) and (outSize), if these sizes are known.
  }

  If you need to decode only part of stream:
  {
    1) try to set partial_decoding mode with
  ICompressSetFinishMode::SetFinishMode(0); 2) Call the Code() function with
  specified (inSize = NULL) and specified (outSize).
  }
  ...
  - ICompressWriteCoderProperties - use it before or after encoding to request
  encoded properties.
...
enum EEnum {
  ...
  kEndMarker,         // VT_BOOL
  kLevel,             // VT_UI4
  kReduceSize, // VT_UI8 : it's estimated size of largest data stream that will
               // be compressed
               //   encoder can use this value to reduce dictionary size and
  ...
  kExpectedDataSize, // VT_UI8 : for ICompressSetCoderPropertiesOpt :
                     ...
                     //   encoder can use this value to optimize encoder
                     //   initialization
  ...
  kAffinity,   // VT_UI8
  /* zstd props */
  kStrategy, // VT_UI4 1=ZSTD_fast, 2=ZSTD_dfast, 3=ZSTD_greedy, 4=ZSTD_lazy,
  ...
  kLong, // VT_UI4 The minimum long is 10 (1KiB) and the maximum is 30 (1GiB) on
         // x32 and 31 (2GiB) on x64
  kWindowLog,  // VT_UI4 The minimum long is 10 (1KiB) and the maximum is 30
               // (1GiB) on x32 and 31 (2GiB) on x64
  kHashLog,    // VT_UI4 The minimum hlog is 6 (64 B) and the maximum is 26 (128
               // MiB).
  kChainLog,   // VT_UI4 The minimum clog is 6 (64 B) and the maximum is 28 (256
               // MiB)
  ...
  kOverlapLog, // VT_UI4 The minimum ovlog is 0 and the maximum is 9.  (default:
               // 6)
  kLdmHashLog, // VT_UI4 The minimum ldmhlog is 6 and the maximum is 26
               // (default: 20).
  kLdmSearchLength,  // VT_UI4 The minimum ldmslen is 4 and the maximum is 4096
                     // (default: 64).
...
CODER_INTERFACE(ICompressSetCoderPropertiesOpt, 0x1F) {
  STDMETHOD(SetCoderPropertiesOpt)
  (const PROPID *propIDs, const PROPVARIANT *props, UInt32 numProps) PURE;
};
...
CODER_INTERFACE(ICompressSetFinishMode, 0x26) {
  ...
  /* finishMode:
    0 : partial decoding is allowed. It's default mode for
    ICompressCoder::Code(), if (outSize) is defined. 1 : full decoding. The
    stream must be finished at the end of decoding. */
};
...
/*
  ...
  to read from inStream to internal buffers more data than minimal data required
  for decoding. So we can call ReadUnusedFromInBuf() from same ICoder object to
  read unused input data from the internal buffer. in ReadUnusedFromInBuf(): the
  Coder is not allowed to use (ISequentialInStream *inStream) object, that was
  sent to ICoder::Code().
*/

CODER_INTERFACE(ICompressReadUnusedFromInBuf, 0x29) {
  STDMETHOD(ReadUnusedFromInBuf)
  (void *data, UInt32 size, UInt32 *processedSize) PURE;
};
...
CODER_INTERFACE(ICompressGetSubStreamSize, 0x30) {
  ...
  /* returns:
    S_OK     : (*value) contains the size or estimated size (can be incorrect
  size) S_FALSE  : size is undefined E_NOTIMP : the feature is not implemented

  Let's (read_size) is size of data that was already read by
  ISequentialInStream::Read(). The caller should call GetSubStreamSize() after
  each Read() and check sizes: if (start_of_subStream + *value < read_size)
    {
      // (*value) is correct, and it's allowed to call GetSubStreamSize() for
  next subStream: start_of_subStream += *value; subStream++;
    }
  */
};
...
CODER_INTERFACE(ICompressSetOutStreamSize, 0x34) {
  ...
  /* That function initializes decoder structures.
     Call this function only for stream version of decoder.
       ...
       if (outSize != NULL), then the decoder must stop decoding after
     (*outSize) bytes. */
};
...
CODER_INTERFACE(ICompressSetInStream2, 0x37) {
  STDMETHOD(SetInStream2)
  (UInt32 streamIndex, ISequentialInStream * inStream) PURE;
  ...
  {
    STDMETHOD(SetOutStream2)(UInt32 streamIndex, ISequentialOutStream
  *outStream) PURE; STDMETHOD(ReleaseOutStream2)(UInt32 streamIndex) PURE;
  };
  ...
  */
  ...
  /*
    ...
    Filter() converts as most as possible bytes required for fast processing.
       ...
       If data stream is not finished, caller must call Filter() for larger
    block: where (size >= smallest_fast_block). if (size >= smallest_fast_block)
       {
         ...
         The caller must read additional data from stream and call Filter()
    again.
       }
       If data stream was finished, caller can call Filter() for (size <
    smallest_fast_block)
...
typedef HRESULT(WINAPI *Func_GetMethodProperty)(UInt32 index, PROPID propID,
                                                PROPVARIANT *value);
typedef HRESULT(WINAPI *Func_CreateDecoder)(UInt32 index, const GUID *iid,
                                            void **outObject);
typedef HRESULT(WINAPI *Func_CreateEncoder)(UInt32 index, const GUID *iid,
                                            void **outObject);

typedef HRESULT(WINAPI *Func_GetHashers)(IHashers **hashers);

typedef HRESULT(WINAPI *Func_SetCodecs)(
